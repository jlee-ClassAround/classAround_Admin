...
import { FreeCourseSchema } from '@/lib/cojooboo/schemas';
...
export const createFreeCourse = async (data: { title: string }) => {
    ...
    const course = await cojoobooDb.freeCourse.create({
        data,
        select: {
            id: true,
        },
    ...
    revalidateTag(`free-courses-FREE`);
...
export const updateFreeCourse = async ({
    ...
    await cojoobooDb.freeCourse.update({
...
export const deleteFreeCourse = async (freeCourseId: string) => {
    ...
    await cojoobooDb.freeCourse.delete({
        where: { id: freeCourseId },
    });
...
export const deleteFreeCourses = async ({ ids }: { ids: string[] }) => {
    ...
    await cojoobooDb.freeCourse.deleteMany({
        where: { id: { in: ids } },
    });
...
export const duplicateFreeCourse = async (freeCourseId: string) => {
    ...
    await cojoobooDb.freeCourse.create({
        data: {
            ...freeCourse,
            id: uuidv4(),

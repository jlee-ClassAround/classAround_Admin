...
import { FreeCourseSchema } from '@/lib/ivy/schemas';
...
export const createFreeCourse = async (data: { title: string }) => {
    ...
    const course = await ivyDb.freeCourse.create({
        data,
        select: {
            id: true,
        },
...
export const deleteFreeCourses = async ({ ids }: { ids: string[] }) => {
    ...
    await ivyDb.freeCourse.deleteMany({
        where: { id: { in: ids } },
    });

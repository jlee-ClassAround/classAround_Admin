...
function ChapterHeader({
    ...
    const onDeleteChapter = () => {
        try {
            if (!confirm('정말로 삭제하시겠습니까? 삭제된 정보는 되돌릴 수 없습니다.')) return;
...
function LessonDragDropContext({
    ...
    return (
        <DragDropContext onDragEnd={(result) => onLessonDragEnd(result, chapterIndex)}>
            <Droppable droppableId={`lessons-${chapter.id}`}>
                {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className="space-y-1">
                        {chapter.lessons.map((lesson, lessonIndex) => (
                            <Draggable key={lesson.id} index={lessonIndex} draggableId={lesson.id}>
                                {(provided) => (
                                    <div
                                        ref={provided.innerRef}
                                        {...provided.draggableProps}

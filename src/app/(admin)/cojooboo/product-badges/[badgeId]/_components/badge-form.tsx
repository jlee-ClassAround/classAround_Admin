...
import { createProductBadgeAction, updateProductBadgeAction } from '../../actions';
...
const badgeSchema = z.object({
    ...
    color: z
        .string()
        .min(1, '배경색을 선택해주세요.')
        .regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, '올바른 HEX 색상 코드를 입력해주세요.'),
    textColor: z
        .string()
...
export function BadgeForm({ initialData }: Props) {
    ...
    const onSubmit = async (values: BadgeSchema) => {
        try {
            ...
            let res;
            if (initialData) {
                res = await updateProductBadgeAction(initialData.id, values);
            } else {
                res = await createProductBadgeAction(values);
            }

            toast.success('정상적으로 처리되었습니다.');
            router.push('/cojooboo/product-badges/all');
            router.refresh();
        } catch {
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormDescription>배지의 배경 색상을 선택해주세요.</FormDescription>
                                <FormMessage />
                            </FormItem>
                        )}
                    />

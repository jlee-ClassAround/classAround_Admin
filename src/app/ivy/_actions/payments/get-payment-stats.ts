...
export async function getPaymentStats({
    ...
    try {
        // 전체 결제 통계 (환불 금액 제외)
        const totalStats = await ivyDb.tossCustomer.aggregate({
            where: whereClause,
            _sum: {
                finalPrice: true,
                cancelAmount: true,
        ...
        // 결제 유형별 통계 - 환불 금액 제외
        const typeStats = await ivyDb.tossCustomer.groupBy({
            where: whereClause,
            by: ['productType'],
            _sum: {
                finalPrice: true,
        ...
        // 쿠폰 사용 통계
        const couponStats = await ivyDb.tossCustomer.aggregate({
            where: {
                AND: [
                    whereClause,
                    {

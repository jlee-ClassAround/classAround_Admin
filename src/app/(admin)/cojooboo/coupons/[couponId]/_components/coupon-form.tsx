...
import { couponSchema, CouponSchema } from '@/lib/cojooboo/schemas';
...
export default function CouponForm({ couponData, courses }: Props) {
    ...
    const onSubmit = async (values: CouponSchema) => {
        try {
            if (!!couponData) {
                await updateCouponAction(couponData.id, values);
            } else {
                await createCouponAction(values);
            }
            toast.success('정상적으로 처리되었습니다.');
            router.push('/cojooboo/coupons/all');
            router.refresh();
        } catch {
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormMessage />
                            <FormDescription>선택에 따라 할인 방식이 달라집니다.</FormDescription>
                        </FormItem>
                    )}
                />
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormMessage />
                            <FormDescription>선택한 시간에 쿠폰이 만료됩니다.</FormDescription>
                        </FormItem>
                    )}
                />

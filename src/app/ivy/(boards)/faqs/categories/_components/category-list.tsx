...
import { IGetCategories } from '@/app/ivy/_actions/categories/get-categories';
...
export function CategoryList({ categories }: Props) {
    ...
    const toggleEdit = (categoryId: string) =>
        setEditCategory(editCategory?.id === categoryId ? null : { id: categoryId });
            ...
            {categories.map((cat) => (
                ...
                >
                    {editCategory?.id === cat.id ? (
                        <form action={action.bind(cat.id)} className="flex items-start gap-1">
                            <input
                                name="categoryId"
                                type="text"
                                defaultValue={cat.id}
                    ...
                    <div className="flex items-center">
                        <Button variant="ghost" size="icon" onClick={() => toggleEdit(cat.id)}>

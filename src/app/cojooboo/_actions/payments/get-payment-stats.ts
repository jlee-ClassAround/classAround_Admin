...
export async function getPaymentStats({
    ...
    try {
        // 전체 결제 통계 (환불 금액 제외)
        const totalStats = await cojoobooDb.tossCustomer.aggregate({
            where: whereClause,
            _sum: {
                finalPrice: true,
                cancelAmount: true,
        ...
        // 결제 유형별 통계 - 환불 금액 제외
        const typeStats = await cojoobooDb.tossCustomer.groupBy({
            where: whereClause,
            by: ['productType'],
            _sum: {
                finalPrice: true,
        ...
        // 쿠폰 사용 통계
        const couponStats = await cojoobooDb.tossCustomer.aggregate({
            where: {
                AND: [
                    whereClause,
                    {
        ...
        const ebookRevenue = typeStats.find((t) => t.productType === 'EBOOK')?._sum.finalPrice || 0;

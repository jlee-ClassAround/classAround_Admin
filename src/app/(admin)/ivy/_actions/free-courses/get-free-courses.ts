...
import { Category, FreeCourse, ProductBadge, Teacher } from '@/generated/ivy';
...
export async function getFreeCourses({
    ...
    try {
        const freeCourses = await ivyDb.freeCourse.findMany({
            where: {
                isPublished: true,
                isHidden: false,
                categoryId,
        ...
        const totalCount = await ivyDb.freeCourse.count({
            where: {
                isPublished: true,
                isHidden: false,
                categoryId,

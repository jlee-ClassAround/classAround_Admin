...
import { sliderSchema, SliderSchema } from '@/lib/cojooboo/schemas';
...
export function SliderForm({ initialData }: Props) {
    ...
    const onSubmit = async (values: SliderSchema) => {
        try {
            if (initialData) {
                await axios.patch(`/api/cojooboo/banners/hero-sliders/${initialData.id}`, values);
            } else {
                await axios.post('/api/cojooboo/banners/hero-sliders', values);
            }

        ...
        async (file: File) => {
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    const phoneColumnName = Object.keys(jsonData[0] as Record<string, any>).find(
                        (key) => ['phone', '전화번호', '연락처', 'tel'].includes(key.toLowerCase())
                    );
                    ...
                    const matchedUsers = users.map((user) => ({
                        id: user.id,
                        label: `${user.username || '이름 없음'} (${user.phone || '휴대폰 없음'}) (${
                            user.email || '이메일 없음'
                        })`,
                    }));

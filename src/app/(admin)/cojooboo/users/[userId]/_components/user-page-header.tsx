    ...
    return (
        <div className="space-y-5 mb-6">
            ...
            <div className="flex items-center gap-x-5 text-sm font-medium">
                <Link
                    href={`/cojooboo/users/${user.id}`}
                    className={cn(
                        'border-b-2 pb-1 border-transparent transition-colors text-gray-500',
                        pathname === `/cojooboo/users/${user.id}` && 'border-primary text-black'
                    )}
                >
                    기본 설정
                </Link>
                <Link
                    href={`/cojooboo/users/${user.id}/courses`}
                    className={cn(
                        'border-b-2 pb-1 border-transparent transition-colors text-gray-500',
                        pathname === `/cojooboo/users/${user.id}/courses` &&

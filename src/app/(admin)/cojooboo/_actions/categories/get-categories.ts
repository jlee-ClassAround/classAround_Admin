...
import { Category, CategoryType } from '@/generated/cojooboo';
...
export async function getCategories({ type }: Props): Promise<IGetCategories[]> {
    try {
        const categories = await cojoobooDb.category.findMany({
            where: {
                type,
            },
            orderBy: {

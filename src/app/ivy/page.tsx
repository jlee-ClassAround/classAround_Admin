...
export default async function AdminDashboard() {
    // 전체 통계 데이터 조회
    const totalUsers = await ivyDb.user.count({
        where: {
            OR: [
                { roleId: null },
                {
    ...
    const orders = await ivyDb.tossCustomer.findMany({
        include: {
            user: {
                select: {
                    username: true,
    ...
    // 총 매출액 계산
    const { totalRevenue } = await getPaymentStats('ivy');
...
async function getUserCountSignedToday() {
    ...
    const todayUsers = await ivyDb.user.count({
        where: {
            createdAt: {
                gte: todayStart,
                lte: todayEnd,
